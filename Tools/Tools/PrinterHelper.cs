...
{
    /// <summary>
    /// GetPrinterStatus : 获取当前指定打印机的状态
    /// DeleteCustomPaperSize:删除已经存在的自定义纸张
    /// AddCustomPaperSize:指定的打印机设置以mm为单位的自定义纸张(Form)
    /// GetPrinterList:获取本地打印机列表
    /// GetDeaultPrinterName:获取本机的默认打印机名称
    /// SetPrinterToDefault:设置默认打印机
    /// PrinterInList:判断打印机是否在系统可用的打印机列表中
    /// FormInPrinter:判断表单是否在指定的打印机所支持的纸张列表中
    /// FormSameSize:判断指定纸张的宽度和高度和与打印内容指定的宽度和高度是否匹配
    /// ============英寸到厘米的转换===========================
    /// FromInchToCM:英寸到厘米的转换
    /// 
    /// SendBytesToPrinter:该方法把非托管内存中的字节数组发送到打印机的打印队列
    /// SendFileToPrinter:发送文件到打印机方法
    /// SendStringToPrinter:将字符串发送到打印机方法
    /// </summary>
